...
class Cidaas_Client_Wrapper {
	...
	private $cookie_token_refresh_key = 'cidaas-refresh';

	// user redirect cookie key
	public $cookie_redirect_key = 'cidaas-redirect';
	 ...
	 * 
	 * @param \Cidaas_Client $client
	 * @param \Cidaas_Option_Settings $settings
	 * @param \Cidaas_Option_Logger $logger
	 */
	function __construct( Cidaas_Client $client, Cidaas_Option_Settings $settings, Cidaas_Option_Logger $logger ){
		$this->client = $client;
	 ...
	 * @param \Cidaas_Option_Logger $logger
	 *
	 * @return \Cidaas_Client_Wrapper
	 */
	static public function register( Cidaas_Client $client, Cidaas_Option_Settings $settings, Cidaas_Option_Logger $logger ){
		...
		add_filter( 'cidaas-alter-request', array( $client_wrapper, 'alter_request' ), 10, 3 );
	...
	function get_end_session_logout_redirect_url( $redirect_url, $requested_redirect_to, $user ) {
		...
		$token_response = $user->get('cidaas-last-token-response');
		if (! $token_response ) {
		...
		$claim = $user->get( 'cidaas-last-id-token-claim' );
	...
	function authentication_request_callback() {
		...
		if ( ! $user ) {
			...
			do_action( 'cidaas-update-user-using-current-claim', $user, $user_claim );
		}
		...
		do_action( 'cidaas-user-logged-in', $user );
		...
		if( $this->settings->redirect_user_back && !empty( $redirect_url ) ) {
			do_action( 'cidaas-redirect-user-back', $redirect_url, $user );
	...
	function login_user( $user, $token_response, $id_token_claim, $user_claim, $subject_identity ){
		...
		update_user_meta( $user->ID, 'cidaas-last-token-response', $token_response );
		update_user_meta( $user->ID, 'cidaas-last-id-token-claim', $id_token_claim );
		update_user_meta( $user->ID, 'cidaas-last-user-claim', $user_claim );
	...
	function get_user_by_identity( $subject_identity ){
		// look for user by their cidaas-subject-identity value
		$user_query = new WP_User_Query( array(
			'meta_query' => array(
				array(
					'key'   => 'cidaas-subject-identity',
	...
	function create_new_user( $subject_identity, $user_claim ) {
		...
		$create_user = apply_filters( 'cidaas-user-creation-test', true, $user_claim );

		if ( ! $create_user ) {
			return new WP_Error( 'cannot-authorize', __( 'Can not authorize.' ), $create_user );
		}
		
		$user_claim = apply_filters( 'cidaas-alter-user-claim', $user_claim );
		$user_data = array(
			'user_login' => $username,
		...
		$user_data = apply_filters( 'cidaas-alter-user-data', $user_data, $user_claim );
		...
		add_user_meta( $user->ID, 'cidaas-subject-identity', (string) $subject_identity, true );

		// log the results
		$this->logger->log( "New user created: {$user->user_login} ($uid)", 'success' );

		// allow plugins / themes to take action on new user creation
		do_action( 'cidaas-user-create', $user, $user_claim );
	...
	function update_existing_user( $uid, $subject_identity ) {
		...
		add_user_meta( $uid, 'cidaas-subject-identity', (string) $subject_identity, true );
		
		// allow plugins / themes to take action on user update
		do_action( 'cidaas-user-update', $uid );
