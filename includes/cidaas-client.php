...
class Cidaas_Client {
	...
	function make_authentication_url() {
		...
		return apply_filters( 'cidaas-auth-url', $url );
	}
	...
	function request_authentication_token( $code ) {
		...
		$request = apply_filters( 'cidaas-alter-request', $request, 'get-authentication-token' );
	...
	function request_new_tokens( $refresh_token ) {
		...
		// allow modifications to the request
		$request = apply_filters( 'cidaas-alter-request', $request, 'refresh-token' );
	...
	function request_userinfo( $access_token ) {
		// allow modifications to the request
		$request = apply_filters( 'cidaas-alter-request', array(), 'get-userinfo' );
	 ...
	 */
	function new_state() {
		$states = get_option( 'cidaas-valid-states', array() );
		...
		// save state
		update_option( 'cidaas-valid-states', $states );
	...
	function validate_user_claim( $user_claim, $id_token_claim ) {
		...
		$login_user = apply_filters( 'cidaas-user-login-test', true, $user_claim );
